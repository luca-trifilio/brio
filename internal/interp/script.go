...
func CollectPreRequestVars(c *canonical.Collection, req *canonical.Request) map[string]string {
	out := map[string]string{}

	// Collection-level script.
	if c != nil && c.Scripts.Pre != "" {
		for k, v := range RunPreRequestScript(c.Scripts.Pre) {
			out[k] = v
		}
	}
	...
	if c != nil && req != nil {
		...
		for i := len(chain) - 1; i >= 0; i-- {
			...
			for k, v := range RunPreRequestScript(f.Scripts.Pre) {
				out[k] = v
			}
		}
	}
	...
	if req != nil && req.Scripts.Pre != "" {
		for k, v := range RunPreRequestScript(req.Scripts.Pre) {
			out[k] = v
		}
	}
