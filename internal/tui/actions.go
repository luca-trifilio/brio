...
func resolveRequest(c *canonical.Collection, env *canonical.Environment, req *canonical.Request, runtime map[string]string) (httpx.ResolvedRequest, *interp.VarScope) {
	...
	case canonical.AuthBearer:
		if auth.Bearer != nil {
			hdr.Set("Authorization", "Bearer "+scope.Interpolate(auth.Bearer.Token))
		}
	case canonical.AuthBasic:
		if auth.Basic != nil {
			req2 := &http.Request{Header: hdr}
			req2.SetBasicAuth(
				scope.Interpolate(auth.Basic.Username),
				scope.Interpolate(auth.Basic.Password),
			)
		}
	case canonical.AuthAPIKey:
		if auth.APIKey != nil {
			key := scope.Interpolate(auth.APIKey.Key)
			val := scope.Interpolate(auth.APIKey.Value)
			switch auth.APIKey.Placement {
			case "query":
				out.QueryParams = append(out.QueryParams, [2]string{key, val})
...
func runRequestCmd(c *canonical.Collection, env *canonical.Environment, req *canonical.Request, runtime map[string]string) tea.Cmd {
	return func() tea.Msg {
		...
		if req.Scripts.Post != "" && resp.Err == nil {
			extracted = interp.RunPostResponseScript(req.Scripts.Post, resp.Body)
		}
		return executeMsg{
			Resp:          resp,
			Resolved:      resolved,
...
func collectionDir(c *canonical.Collection) string {
	if c == nil {
		return ""
	}
