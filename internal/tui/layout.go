...
func (m *Model) renderLayout() string {
	...
	if m.showPicker {
		body = overlay(body, m.picker.View(m.width/2, m.height-8), m.width, m.height)
	}
	if m.showDiag {
		body = overlay(body, m.diagPane.View(m.width-12, m.height-8), m.width, m.height)
	}
	if m.showCollMgr {
		body = overlay(body, m.collMgr.View(m.width-12, m.height-8), m.width, m.height)
	}
