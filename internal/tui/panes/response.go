...
type ResponseModel struct {
	...
	// jq filter state
	filterMode  bool            // true while typing a jq filter
	filterInput textinput.Model // used only for rendering the cursor/prompt
	filterBuf   string          // characters typed so far (source of truth)
	filterQuery string          // last-applied query (empty = no filter)
	filterBody  []byte          // filtered body (when filterQuery is non-empty and successful)
	filterErr   string          // last filter error (rendered in bottom hint)
}

func NewResponse() *ResponseModel {
	ti := textinput.New()
	ti.Prompt = "| "
	ti.CharLimit = 256
	ti.Width = 60
	return &ResponseModel{searchForward: true, filterInput: ti}
}

// Searching reports whether the user is in any input mode that should absorb keys.
func (r *ResponseModel) inputMode() bool { return r.searching || r.leaping || r.filterMode }

// FilterError returns the last jq filter error message ("" when none).
func (r *ResponseModel) FilterError() string { return r.filterErr }

// applyJQ evaluates query against jsonBody. The first emitted result is
// pretty-printed and returned. On any error the original body is returned
// alongside the error. A panic from gojq is recovered and returned as error.
func applyJQ(jsonBody []byte, query string) (out string, err error) {
	defer func() {
		if r := recover(); r != nil {
...
// Searching returns true while the user is in search, leap, or filter input mode.
func (r *ResponseModel) Searching() bool { return r.searching || r.leaping || r.filterMode }
...
func (r *ResponseModel) rebuild() {
	...
	resp := r.resp
	if resp != nil && len(r.filterBody) > 0 {
		// Render the filtered JSON in place of the original body.
		clone := *resp
		clone.Body = r.filterBody
		resp = &clone
	}
	pre := buildLines(resp, r.resolved, r.width)
	nLines := len(pre)
	if nLines < 1 {
		nLines = 1
	}
	...
	r.lines = buildLines(resp, r.resolved, r.contentW)
	r.recomputeMatches()
}

func (r *ResponseModel) applyFilter() {
	r.filterErr = ""
	r.filterBody = nil
	if r.filterQuery == "" || r.resp == nil {
		return
	}
	body := r.resp.Body
	out, err := applyJQ(body, r.filterQuery)
	if err != nil {
		r.filterErr = err.Error()
		return
	}
	r.filterBody = []byte(out)
}
...
func (r *ResponseModel) HandleKey(key string) bool {
	...
	// jq filter input mode.
	if r.filterMode {
		switch key {
		case "enter":
			q := strings.TrimSpace(r.filterBuf)
			r.filterMode = false
			r.filterInput.Blur()
			r.filterQuery = q
			r.applyFilter()
		...
		case "backspace", "ctrl+h":
			if len(r.filterBuf) > 0 {
				r.filterBuf = r.filterBuf[:len(r.filterBuf)-1]
				r.filterInput.SetValue(r.filterBuf)
			}
		default:
			if len(key) == 1 {
				r.filterBuf += key
	...
	case "|":
		if r.resp != nil && len(r.resp.Body) > 0 {
			r.filterMode = true
			r.filterBuf = r.filterQuery
			r.filterInput.SetValue(r.filterBuf)
			r.filterInput.CursorEnd()
			r.filterInput.Focus()
		}
	case "/":
		r.searching = true
...
func (r *ResponseModel) View(width, height int, focused bool) string {
	...
	case r.filterMode:
		bottomLine = lipgloss.NewStyle().Foreground(theme.Mauve).Render(r.filterInput.View()) + "█"
	case r.filterErr != "":
		bottomLine = theme.StyleError.Render("jq: "+r.filterErr) +
			theme.StyleDim.Render("  | edit  esc clear")
	case r.filterQuery != "":
		bottomLine = theme.StyleSuccess.Render("| "+r.filterQuery) +
			theme.StyleDim.Render(fmt.Sprintf("  %d lines  | edit", len(r.lines)))
	case r.searching:
		prefix := "/"
		if !r.searchForward {
			prefix = "?"
