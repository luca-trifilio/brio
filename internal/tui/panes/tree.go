...
func NewTree(cs []*canonical.Collection) *TreeModel {
...
func (t *TreeModel) Rebuild() {
	...
	for ix, c := range t.Collections {
		...
		t.rows = append(t.rows, TreeNode{
			...
			Path:         c.Root,
			Expandable:   true,
			Expanded:     t.Expanded[c.Root],
		})
		if !t.Expanded[c.Root] {
			continue
		}
		if c.RootFolder != nil {
			t.appendFolder(c.RootFolder, ix, 1, true)
		}
	}
...
func (t *TreeModel) folderHasUsable(f *canonical.Folder) bool {
...
func (t *TreeModel) appendFolder(f *canonical.Folder, ix, depth int, isRoot bool) {
	if !isRoot {
